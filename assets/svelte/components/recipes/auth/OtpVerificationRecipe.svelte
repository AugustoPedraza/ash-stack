...
<div class="h-full bg-base-100 overflow-y-auto px-5 py-6">
  {#if success}
    <div class="flex flex-col items-center justify-center min-h-[80vh] text-center">
      ...
      <h1 class="text-xl font-semibold text-foreground mb-2">Verified!</h1>
      <p class="text-muted-foreground mb-8">Your phone number has been verified.</p>
  ...
  {:else}
    <div class="max-w-sm mx-auto pt-10">
      <div class="text-center mb-8">
        <h1 class="text-xl font-semibold text-foreground mb-2">Enter Code</h1>
        <p class="text-muted-foreground">We sent a 6-digit code to your phone.</p>
        {#if expiresIn > 0}
          <p class="text-sm text-text-disabled mt-2">Expires in {formattedTime}</p>
        {:else}
          <p class="text-sm text-destructive mt-2">Code expired</p>
        {/if}
      </div>

      {#if error}
        <div class="flex items-center justify-center gap-2 p-3 bg-destructive/10 border border-destructive/30 rounded-lg text-destructive text-sm mb-6">
