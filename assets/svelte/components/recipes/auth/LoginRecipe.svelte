...
<div class="h-full bg-base-100 overflow-y-auto px-5 py-6">
  {#if success}
    ...
    <div class="flex flex-col items-center justify-center min-h-[80vh] text-center">
      ...
      <h1 class="text-xl font-semibold text-text mb-2">Welcome Back!</h1>
      <p class="text-text-secondary mb-8">You've successfully signed in.</p>
  ...
  {:else}
    ...
    <div class="max-w-sm mx-auto pt-10">
      ...
      <div class="text-center mb-8">
        <h1 class="text-xl font-semibold text-text mb-2">Sign In</h1>
        <p class="text-text-muted">Welcome back! Please sign in to continue.</p>
      </div>
      ...
      <form class="flex flex-col gap-5" onsubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
        ...
        <div class="flex flex-col gap-1.5">
          ...
          {#if touched.password && (fieldErrors.password || !passwordValid)}
            <span class="text-xs text-error">{fieldErrors.password || 'Password must be at least 6 characters'}</span>
          {/if}
        </div>
