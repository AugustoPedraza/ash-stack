...
<div class="h-full bg-background overflow-y-auto px-5 py-6">
  <div class="max-w-md mx-auto">
    <div class="mb-8">
      <h1 class="text-xl font-semibold text-foreground mb-2">Back Navigation</h1>
      <p class="text-muted-foreground text-sm">Mobile standard: simple back link instead of breadcrumb trail.</p>
    </div>

    <div class="flex flex-col gap-6">
      <!-- Mobile Back Pattern (Primary) -->
      <div class="flex flex-col gap-1.5">
        <span class="text-sm font-medium text-foreground">Standard Back Link</span>
        <p class="text-xs text-muted-foreground mb-2">Used in screen headers or at top of content.</p>
      ...
      <div class="flex flex-col gap-1.5">
        <span class="text-sm font-medium text-foreground">In Screen Header</span>
        <p class="text-xs text-muted-foreground mb-2">Back button integrated with page title.</p>
        ...
        <div class="border border-border-strong rounded-lg bg-background overflow-hidden">
          ...
          <div class="flex items-center justify-between px-4 py-3 border-b border-border">
            <div class="flex items-center gap-3">
              {#if previousPage}
                <button
                  type="button"
                  class="w-8 h-8 flex items-center justify-center rounded-full text-muted-foreground hover:text-foreground hover:bg-surface-sunken transition-colors"
                  onclick={goBack}
                  aria-label="Go back"
                >
              ...
              {/if}
              <h2 class="text-base font-semibold text-foreground">{currentPage.label}</h2>
            </div>
          ...
          <!-- Content area -->
          <div class="p-4">
            <p class="text-sm text-muted-foreground">Page content for {currentPage.label}</p>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-1.5">
        <span class="text-sm font-medium text-foreground">Simulate Navigation</span>
        <div class="flex items-center justify-between">
          <button
            type="button"
            class="text-sm text-muted-foreground hover:text-foreground transition-colors disabled:opacity-30"
            onclick={goBack}
      ...
      <div class="flex flex-col gap-1.5">
        <span class="text-sm font-medium text-foreground">Path Context</span>
        <p class="text-xs text-muted-foreground mb-2">Show location context without making it clickable.</p>
      ...
      <div class="p-4 bg-surface-sunken rounded-lg">
        <h3 class="text-sm font-medium text-foreground mb-2">Navigation State</h3>
