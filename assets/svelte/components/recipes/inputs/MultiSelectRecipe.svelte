...
<div class="h-full bg-surface overflow-y-auto px-5 py-6">
  <div class="max-w-md mx-auto">
    <div class="mb-8">
      <h1 class="text-xl font-semibold text-text mb-2">Multi-Select</h1>
      <p class="text-text-muted text-sm">Select multiple options from a dropdown list.</p>
    </div>
    ...
    <div class="flex flex-col gap-6">
      ...
      <div class="flex flex-col gap-1.5">
        <div class="flex items-center justify-between">
          <span class="text-sm font-medium text-text">Technologies</span>
          <span class="text-xs text-text-muted">{selected.length}/{MAX_ITEMS}</span>
        </div>
        ...
        <div class="relative">
          ...
          >
            <span class="{selected.length > 0 ? 'text-text' : 'text-text-muted'}">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-text-muted transition-transform {showDropdown ? 'rotate-180' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <div class="absolute top-full left-0 right-0 mt-1 bg-surface border border-border-strong rounded-lg shadow-lg z-20 overflow-hidden">
              ...
              <div class="px-3 py-2 border-b border-border">
                <input
                  bind:this={inputRef}
                  type="text"
                  class="w-full text-sm bg-transparent outline-none placeholder:text-text-disabled text-text"
                  placeholder="Search..."
        ...
        {#if isAtMax}
          <p class="text-xs text-text-muted">Maximum {MAX_ITEMS} items</p>
        {/if}
      </div>
      ...
      {#if selected.length > 0}
        <div class="flex flex-col gap-1.5">
          <span class="text-sm font-medium text-text">Selected</span>
          <div class="flex flex-wrap gap-2">
            {#each selected as item}
              <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-surface-sunken text-text rounded-lg text-sm">
                {item.label}
                <button
                  type="button"
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <div class="pt-6 border-t border-border">
        <h3 class="text-sm font-medium text-text mb-3">Alternative: Inline Checkboxes</h3>
        <p class="text-xs text-text-muted mb-3">For shorter lists, inline checkboxes can be clearer.</p>
      ...
      {#if selected.length > 0}
        <div class="p-4 bg-surface-sunken rounded-lg">
          <h3 class="text-sm font-medium text-text mb-2">Form Value</h3>
          <pre class="text-xs text-text-secondary overflow-x-auto">{JSON.stringify(selected.map(s => s.id), null, 2)}</pre>
        </div>
      {/if}
    </div>
  </div>
