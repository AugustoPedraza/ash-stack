...
<div class="h-full bg-surface overflow-y-auto px-5 py-6">
  <div class="max-w-md mx-auto">
    <div class="mb-8">
      <h1 class="text-xl font-semibold text-text mb-2">Searchable Select</h1>
      <p class="text-text-muted text-sm">Async search dropdown with suggestions.</p>
    </div>

    <!-- Single Select -->
    <div class="mb-8">
      <span id="country-label" class="block text-sm font-medium text-text mb-2">Select Country</span>
      <div bind:this={containerElement} class="relative">
        <!-- Trigger -->
        <div class="flex items-center gap-2">
          <div
            class="flex-1 flex items-center gap-2 px-4 py-3 bg-surface border border-border-strong rounded-xl hover:border-text/30 transition-colors cursor-pointer
          ...
          >
            {#if selectedOption}
              ...
              <span class="flex-1 text-text">{selectedOption.label}</span>
            {:else}
        ...
        <!-- Dropdown -->
        {#if isOpen}
          <div class="absolute z-10 w-full mt-2 bg-surface border border-border rounded-xl shadow-lg overflow-hidden">
            <!-- Search Input -->
            <div class="p-2 border-b border-border">
              <div class="flex items-center gap-2 px-3 py-2 bg-surface-sunken/50 rounded-lg">
                ...
                <input
                  ...
                  class="flex-1 bg-transparent text-sm text-text placeholder:text-text-disabled outline-none"
                />
                {#if loading}
            ...
            <div class="max-h-60 overflow-y-auto p-1" role="listbox">
              {#if filteredOptions.length === 0}
                <div class="px-4 py-8 text-center">
                  <p class="text-sm text-text-muted">No countries found</p>
                </div>
              {:else}
    ...
    <div class="mb-8">
      <h2 class="text-sm font-medium text-text mb-4">Multi-Select Preview</h2>
      <div class="p-4 bg-surface-sunken/50 rounded-xl">
        <div class="flex flex-wrap gap-2 p-3 bg-surface border border-border-strong rounded-lg min-h-[48px]">
          <!-- Sample chips -->
        ...
        <p class="text-xs text-text-muted mt-2">Multi-select with chip display</p>
      </div>
    </div>
