...
<div class="h-full bg-background overflow-y-auto px-5 py-6">
  <div class="max-w-md mx-auto">
    <div class="mb-8">
      <h1 class="text-xl font-semibold text-foreground mb-2">Badge Counters</h1>
      <p class="text-muted-foreground text-sm">Animated badges with various styles and interactions.</p>
    </div>

    <!-- Style 1: Icon Badges -->
    <div class="mb-8">
      <h2 class="text-sm font-medium text-foreground mb-4">Icon Badges</h2>
      <div class="flex items-center gap-6 p-4 bg-surface-sunken/50 rounded-xl">
        ...
        <div class="relative">
          ...
          {#if counts.notifications > 0}
            <span
              class="absolute -top-1 -right-1 min-w-5 h-5 px-1 flex items-center justify-center text-xs font-medium text-white bg-destructive rounded-full transition-transform {animating.notifications ? 'scale-125' : 'scale-100'}"
            >
        ...
        <div class="relative">
          ...
          {#if counts.alerts > 0}
            <span
              class="absolute -top-1 -right-1 min-w-5 h-5 px-1 flex items-center justify-center text-xs font-medium text-white bg-destructive rounded-full transition-transform {animating.alerts ? 'scale-125' : 'scale-100'}"
            >
    ...
    <div class="mb-8 p-4 bg-surface-sunken rounded-xl">
      <p class="text-sm font-medium text-foreground mb-3">Demo Controls</p>
    ...
    <!-- Style 2: Dot Badges -->
    <div class="mb-8">
      <h2 class="text-sm font-medium text-foreground mb-4">Dot Badges (Unread Indicator)</h2>
      <div class="space-y-2 p-4 bg-surface-sunken/50 rounded-xl">
        <div class="flex items-center gap-3 p-2 rounded-lg hover:bg-surface-sunken">
          ...
          <div class="flex-1">
            <p class="text-sm font-medium text-foreground">Sarah Chen</p>
            <p class="text-xs text-muted-foreground">Sent you a message</p>
          </div>
        </div>
        <div class="flex items-center gap-3 p-2 rounded-lg hover:bg-surface-sunken">
          <div class="relative">
            ...
            {#if counts.notifications > 0}
              <span class="absolute top-0 right-0 w-3 h-3 bg-destructive rounded-full ring-2 ring-background"></span>
            {/if}
          </div>
          <div class="flex-1">
            <p class="text-sm font-medium text-foreground">Mike Johnson</p>
            <p class="text-xs text-muted-foreground">Mentioned you in a comment</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="mb-8">
      <h2 class="text-sm font-medium text-foreground mb-4">Tab Bar Style</h2>
      <div class="flex justify-around p-4 bg-surface-sunken/50 rounded-xl">
        ...
        <button type="button" class="flex flex-col items-center gap-1 p-2 relative" onclick={() => decrement('notifications')}>
          <div class="relative">
            ...
            {#if counts.notifications > 0}
              <span class="absolute -top-2 -right-2 min-w-4 h-4 px-1 flex items-center justify-center text-xs font-medium text-white bg-destructive rounded-full">
                {formatCount(counts.notifications)}
              </span>
            {/if}
          </div>
    ...
    <div class="mb-8">
      <h2 class="text-sm font-medium text-foreground mb-4">List Item Badges</h2>
      <div class="bg-background border border-border rounded-xl divide-y divide-border">
        ...
        <button type="button" class="w-full flex items-center justify-between p-4 hover:bg-surface-sunken/50 text-left" onclick={() => clear('notifications')}>
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-foreground">Updates</span>
          </div>
          {#if counts.notifications > 0}
            <span class="px-2 py-0.5 text-xs font-medium text-destructive bg-destructive/15 rounded-full">
              {formatCount(counts.notifications)}
            </span>
          {/if}
        </button>
        <button type="button" class="w-full flex items-center justify-between p-4 hover:bg-surface-sunken/50 text-left" onclick={() => clear('tasks')}>
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-foreground">Archive</span>
          </div>
        </button>
      </div>
    </div>
