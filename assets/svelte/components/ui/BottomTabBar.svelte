...
<nav class="bg-base-100 border-t border-base-300 pb-safe">
  <div class="flex justify-around">
    {#each tabs as tab}
      <button
        class="flex flex-col items-center gap-1 px-4 py-2 min-w-[64px] transition-colors relative
          {activeTab === tab.id ? 'text-primary' : 'text-base-content/50 hover:text-base-content/70'}"
      ...
      >
        <div class="relative">
          ...
          {#if tab.badge && tab.badge > 0}
            <span class="absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] flex items-center justify-center text-[10px] font-bold bg-error text-white rounded-full px-1">
